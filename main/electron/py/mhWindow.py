...
class MHWindow:
    ...
    def F_行囊拿红蓝(self, pic):
        ...
        if (result != None):
            ...
            utils.click()
            time.sleep(0.2)
            self.pointMove(self.daojuArea[0] + 5, self.daojuArea[1] + 226)
            time.sleep(0.2)
            utils.click()
            time.sleep(0.2)
            utils.click()
    ...
    def F_判断人物宝宝低红蓝位(self):
        self.F_打开道具()
        pyautogui.press('f7')
        # 检查红
        ret = baiduApi.op.FindMultiColor(
            self.windowArea[0] + 775, self.windowArea[1] + 10, self.windowArea[0] + 775 + 15, self.windowArea[1] + 10 + 7, 'b89090', '3|2|804c48', 0.7, 0)
        if(ret[1] > 0):
            self.F_吃红()
            print('人物缺红')
        # 检查蓝
        ...
        if(ret[1] > 0):
            ...
            print('人物缺蓝')
        # point = self.findImgInWindow(
        #     'all-baobao-hong.png', confidence=0.90, area=(639, 10, 20, 20))
        # if point != None:
        #     print('宝宝缺红')
    ...
    def 统计扫货(self):
        ...
        if(point != None):
            return '内丹'
        point = self.findImgInWindow('all-shoujue-big.png',  0.75, [self.windowArea[0] + 80, self.windowArea[1],
                                                                    520, 480])
        if(point != None):
            return '兽决'
        point = self.findImgInWindow('all-66-big.png',  0.85, [self.windowArea[0] + 80, self.windowArea[1],
                                                               520, 480])
        if(point != None):
            return '金柳露'
    ...
    def F_判断是否有飞行符(self):
        ...
        if (result == None):
            ...
            if point != None:
                ...
                utils.click()
                self.pointMove(
                    self.daojuArea[0] + 5, self.daojuArea[1] + 226)
                time.sleep(0.5)
                utils.click()
                time.sleep(0.5)
                utils.click()
...
if __name__ == '__main__':
    ...
    point = window.findImgInWindow(
        'all-baobao-hong.png', confidence=0.85, area=(639, 10, 20, 20))
    if point != None:
        print('宝宝缺红')
        window.F_移动到游戏区域坐标(620, 100)
        window.F_移动到游戏区域坐标(620, 50)
        window.F_移动到游戏区域坐标(630, 12)
        utils.rightClick()
