...
export const doGetWatuInfo = (deviceId: number, cangkuPath: string) => (window as any).ipcRenderer.sendSync(resourcePaths.METHOD_GET_WATU_INFO, [deviceId, cangkuPath])
...
export const doGetWatuClickMap = (deviceId: number, mapName: string, x: number, y: number, index: number, otherPoint?: string, isBeen?: boolean, cangkuPath?: string) => (window as any).ipcRenderer.sendSync(resourcePaths.METHOD_CLICK_WATU_MAP, [deviceId, mapName, x, y, index, otherPoint, isBeen, cangkuPath])
export const doBee = (deviceId: number, cangkuPath: string) => (window as any).ipcRenderer.sendSync(resourcePaths.METHOD_BEE_MODE, [deviceId, cangkuPath])
